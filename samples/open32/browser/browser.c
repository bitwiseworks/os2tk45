                   ...
                   int nCmdShow)
{
  ...
  hClientMenu = LoadMenu(hInst, "BROWSERM");    /*   ... Client             */
  hChildMenu = LoadMenu(hInst, "BROWSFILEM");   /*   ... Child              */
    ...
    case WM_CREATE:
      ...
      {                                          /* create child wndw immed. */
        ...
        SetClassLong(hChildWnd, GCL_HICON,
                              (LONG) LoadIcon(hInstance, "IDI_WIZZARD"));
      }
      return 0;
      break;
  ...
  switch (message)
  {
    ...
    /************************************************************************/
      ...
      if (hFile != INVALID_HANDLE_VALUE)        /* No error on file open... */
      {
        ...
        pFilePath = GlobalAlloc(GPTR, FileSize); /* Allocate memory for file*/
        ...
        {                                        /* Read the file...        */
          if (ReadFile(hFile, pFilePath, FileSize , &FileRead, NULL))
          {
            ...
            SetProp(hWnd, "FILE_DATA",
                    pFilePath);                 /* Save @ to data as a child*/
                                                /* window property.         */
          }
          else                                  /* Read error...            */
          {
      ...
      psd = LocalAlloc(LPTR, sizeof(struct scrolldata));
      SetProp(hWnd, "SCROLL_DATA", psd);        /* Save address as wndw prop*/
                                                /* Get number of lines      */
    ...
    case WM_SIZE:
      psd = GetProp(hWnd, "SCROLL_DATA");       /* Get scroll info for wndw */
    ...
    case WM_VSCROLL:
      psd = GetProp(hWnd, "SCROLL_DATA");       /* Get scroll info for wndw */
      pFilePath = GetProp(hWnd, "FILE_DATA");   /* Get ptr to file data.    */
    ...
    case WM_PAINT:
      psd = GetProp(hWnd, "SCROLL_DATA");       /* Get scroll info for wndw */
      pFilePath = GetProp(hWnd, "FILE_DATA");   /* Get ptr to data to paint */
    ...
    case WM_DESTROY:                   /* Free the memory belonging to child*/
      ...
      hFileName = (char*)GetWindowLong(hWnd, GWL_USERDATA); /* instance data*/
      LocalFree(hFileName);
